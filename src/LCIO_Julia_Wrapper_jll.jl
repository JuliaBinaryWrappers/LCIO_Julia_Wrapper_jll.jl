...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import LCIO_Julia_Wrapper_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"LCIO_Julia_Wrapper"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("LCIO_Julia_Wrapper_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "LCIO_Julia_Wrapper_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"LCIO_Julia_Wrapper", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "LCIO_Julia_Wrapper_jll", "src", "LCIO_Julia_Wrapper_jll.jl"))
        @info("LCIO_Julia_Wrapper_ll dev'ed out to /depot/dev/LCIO_Julia_Wrapper_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("1fe59b3e-ae39-56b2-a121-c4c5e6ddab56"))
platforms = [Pkg.Artifacts.unpack_platform(e, "LCIO_Julia_Wrapper", artifacts_toml) for e in artifacts["LCIO_Julia_Wrapper"]]
...
if best_platform === nothing
    @debug("Unable to load LCIO_Julia_Wrapper; unsupported platform $(triplet(platform_key_abi()))")
else
